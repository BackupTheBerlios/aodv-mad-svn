...
static void kaodv_netlink_rcv_sk(struct sk_buff *skb)
{
	...
	printk(KERN_ERR "kaodv_netlink_rc_sk: skb!\n");
...
#else
	kaodvnl = netlink_kernel_create(&init_net, NETLINK_AODV, AODVGRP_MAX, kaodv_netlink_rcv_sk, NULL, THIS_MODULE);
	printk(KERN_ERR "kaodv_netlink: created!\n");
#endif
	if (kaodvnl == NULL) {
